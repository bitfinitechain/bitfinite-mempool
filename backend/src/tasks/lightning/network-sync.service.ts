...
class NetworkSyncService {
  ...
  private async $runTasks(): Promise<void> {
    ...
    try {
      ...
      if (networkGraph.nodes.length === 0 || networkGraph.edges.length === 0) {
        logger.info(
          `LN Network graph is empty, retrying in 10 seconds`,
          logger.tags.ln
        );
        setTimeout(() => {
          this.$runTasks();
      ...
      logger.err(
        `$runTasks() error: ${e instanceof Error ? e.message : e}`,
        logger.tags.ln
      );
    }
    ...
    setTimeout(
      ...
      Math.max(
        1,
        1000 * config.LIGHTNING.GRAPH_REFRESH_INTERVAL -
  ...
  private async $updateNodesList(nodes: ILightningApi.Node[]): Promise<void> {
    ...
    for (const node of nodes) {
      const latestUpdated = await channelsApi.$getLatestChannelUpdateForNode(
        node.pub_key
      );
      ...
      if (elapsedSeconds > config.LIGHTNING.LOGGER_UPDATE_INTERVAL) {
        logger.debug(
          `Updating node ${progress}/${nodes.length}`,
          logger.tags.ln
        );
        this.loggerTimer = new Date().getTime() / 1000;
      }
      ...
      for (const socket of node.addresses) {
        await NodesSocketsRepository.$saveSocket(
          Common.formatSocket(node.pub_key, socket)
        );
        addresses.push(socket.addr);
      }
      deletedSockets += await NodesSocketsRepository.$deleteUnusedSockets(
        node.pub_key,
        addresses
      );

      const oldRecordTypes = await NodeRecordsRepository.$getRecordTypes(
        node.pub_key
      );
      ...
      if (
        oldRecordTypes.reduce(
          (changed, type) => changed || customRecordTypes.indexOf(type) === -1,
          false
        )
      ) {
        deletedRecords += await NodeRecordsRepository.$deleteUnusedRecords(
          node.pub_key,
          customRecordTypes
        );
      }
    ...
    logger.debug(
      `${progress} nodes updated. ${deletedSockets} sockets deleted. ${deletedRecords} custom records deleted.`
    );
   ...
   */
  private async $updateChannelsList(
    channels: ILightningApi.Channel[]
  ): Promise<void> {
    try {
      const [closedChannelsRaw]: any[] = await DB.query(
        `SELECT id FROM channels WHERE status = 2`
      );
      ...
      for (const channel of channels) {
        ...
        if (elapsedSeconds > config.LIGHTNING.LOGGER_UPDATE_INTERVAL) {
          logger.debug(
            `Updating channel ${progress}/${channels.length}`,
            logger.tags.ln
          );
          this.loggerTimer = new Date().getTime() / 1000;
        }
      ...
      logger.err(
        ` Cannot update channel list. Reason: ${
  ...
  private async $updateNodeFirstSeen(): Promise<void> {
    ...
    try {
      ...
      for (const node of nodes) {
        ...
        if (elapsedSeconds > config.LIGHTNING.LOGGER_UPDATE_INTERVAL) {
          logger.debug(
            `Updating node first seen date ${progress}/${nodes.length}`,
            logger.tags.ln
          );
          this.loggerTimer = new Date().getTime() / 1000;
        }
      ...
      if (updated > 0) {
        logger.debug(
          `Updated ${updated} node first seen dates`,
          logger.tags.ln
        );
      }
    } catch (e) {
      logger.err(
        `$updateNodeFirstSeen() error: ${e instanceof Error ? e.message : e}`,
        logger.tags.ln
      );
    }
  }
  ...
  private async $lookUpCreationDateFromChain(): Promise<void> {
    ...
    try {
      ...
      for (const channel of channels) {
        const transaction = await fundingTxFetcher.$fetchChannelOpenTx(
          channel.short_id
        );
        if (!transaction) {
          continue;
        ...
        if (elapsedSeconds > config.LIGHTNING.LOGGER_UPDATE_INTERVAL) {
          logger.debug(
            `Updating channel creation date ${progress}/${channels.length}`,
            logger.tags.ln
          );
          this.loggerTimer = new Date().getTime() / 1000;
        }
      ...
      if (channels.length > 0) {
        logger.debug(
          `Updated ${channels.length} channels' creation date`,
          logger.tags.ln
        );
      }
    } catch (e) {
      logger.err(
        `$lookUpCreationDateFromChain() error: ${
          e instanceof Error ? e.message : e
        }`,
        logger.tags.ln
      );
  ...
  private async $deactivateChannelsWithoutActiveNodes(): Promise<void> {
    ...
    try {
      ...
      if (result[0].changedRows ?? 0 > 0) {
        logger.debug(
          `Marked ${result[0].changedRows} channels as inactive because they are not linked to any active node`,
          logger.tags.ln
        );
      }
    } catch (e) {
      logger.err(
        `$deactivateChannelsWithoutActiveNodes() error: ${
          e instanceof Error ? e.message : e
        }`,
        logger.tags.ln
      );
  ...
  private async $scanForClosedChannels(): Promise<void> {
    ...
    try {
      if (config.MEMPOOL.ENABLED === false) {
        // https://github.com/mempool/mempool/issues/3582
        currentBlockHeight = await bitcoinApi.$getBlockHeightTip();
      }
      if (this.closedChannelsScanBlock === currentBlockHeight) {
        logger.debug(
          `We've already scan closed channels for this block, skipping.`
        );
        return;
      }
      ...
      for (let i = 0; i < Math.ceil(allChannels.length / sliceLength); i++) {
        ...
        const outspends = await bitcoinApi.$getOutSpendsByOutpoint(
          channels.map((channel) => {
            return {
              txid: channel.transaction_id,
              vout: channel.transaction_vout,
            };
          })
        );
        ...
        for (const [index, channel] of channels.entries()) {
          ...
          if (
            spendingTx.spent === true &&
            spendingTx.status?.confirmed === true
          ) {
            // logger.debug(`Marking channel: ${channel.id} as closed.`, logger.tags.ln);
            await DB.query(
              `UPDATE channels SET status = 2, closing_date = FROM_UNIXTIME(?) WHERE id = ?`,
              [spendingTx.status.block_time, channel.id]
            );
            if (spendingTx.txid && !channel.closing_transaction_id) {
              await DB.query(
                `UPDATE channels SET closing_transaction_id = ? WHERE id = ?`,
                [spendingTx.txid, channel.id]
              );
            }
          }
        }
        ...
        if (elapsedSeconds > config.LIGHTNING.LOGGER_UPDATE_INTERVAL) {
          logger.debug(
            `Checking if channel has been closed ${progress}/${allChannels.length}`,
            logger.tags.ln
          );
          this.loggerTimer = new Date().getTime() / 1000;
        }
      ...
      logger.debug(
        `Closed channels scan completed at block ${this.closedChannelsScanBlock}`,
        logger.tags.ln
      );
    } catch (e) {
      logger.err(
        `$scanForClosedChannels() error: ${e instanceof Error ? e.message : e}`,
        logger.tags.ln
      );
    }
  }
