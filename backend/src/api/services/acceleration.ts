...
class AccelerationApi {
  ...
  private useWebsocket: boolean =
    config.MEMPOOL.OFFICIAL && config.MEMPOOL_SERVICES.ACCELERATIONS;
  private startedWebsocketLoop: boolean = false;
  private websocketConnected: boolean = false;
  private onDemandPollingEnabled = !config.MEMPOOL_SERVICES.ACCELERATIONS;
  private apiPath = config.MEMPOOL.OFFICIAL
    ? config.MEMPOOL_SERVICES.API + '/accelerator/accelerations'
    : config.EXTERNAL_DATA_SERVER.MEMPOOL_API + '/accelerations';
  private websocketPath = config.MEMPOOL_SERVICES?.API
    ? `${config.MEMPOOL_SERVICES.API.replace('https://', 'wss://').replace(
        'http://',
        'ws://'
      )}/accelerator/ws`
    : '/';
  ...
  private myAccelerations: Record<
    string,
    { status: MyAccelerationStatus; added: number; acceleration?: Acceleration }
  > = {};
  ...
  public countMyAccelerationsWithStatus(filter: MyAccelerationStatus): number {
    return Object.values(this.myAccelerations).reduce((count, { status }) => {
      return count + (status === filter ? 1 : 0);
    }, 0);
  }
  ...
  private async $fetchAccelerations(): Promise<Acceleration[] | null> {
    try {
      ...
      logger.warn(
        'Failed to fetch current accelerations from the mempool services backend: ' +
  ...
  public async $updateAccelerations(): Promise<Record<
    string,
    Acceleration
  > | null> {
  ...
  private async $updateAccelerationsOnDemand(): Promise<Record<
    string,
    Acceleration
  > | null> {
    const shouldUpdate =
      ...
      (this.countMyAccelerationsWithStatus('accelerating') > 0 &&
        this.lastPoll < Date.now() - 10 * 60 * 1000);
    ...
    if (shouldUpdate) {
      ...
      if (accelerations) {
        ...
        for (const [txid, { status, acceleration }] of Object.entries(
          this.myAccelerations
        )) {
          if (status === 'accelerating' && !latestAccelerations[txid]) {
            this.myAccelerations[txid] = {
              status: 'done',
              added: Date.now(),
              acceleration,
            };
    ...
    for (const [txid, { status, added }] of Object.entries(
      this.myAccelerations
    )) {
      if (
        ['requested', 'done'].includes(status) &&
        added < Date.now() - 1000 * 60 * 10
      ) {
    ...
    const latestAccelerations = {};
    for (const acc of Object.values(this.myAccelerations)
      .map(({ acceleration }) => acceleration)
      .filter((acc) => acc) as Acceleration[]) {
      latestAccelerations[acc.txid] = acc;
    }
    this._accelerations = latestAccelerations;
    return this._accelerations;
  ...
  public async $fetchAccelerationHistory(
    page?: number,
    status?: string
  ): Promise<AccelerationHistory[] | null> {
    if (config.MEMPOOL_SERVICES.ACCELERATIONS) {
      try {
        const response = await axios.get(
          `${config.MEMPOOL_SERVICES.API}/accelerator/accelerations/history`,
          {
            responseType: 'json',
            timeout: 10000,
            params: {
        ...
        logger.warn(
          'Failed to fetch acceleration history from the mempool services backend: ' +
            (e instanceof Error ? e.message : e)
        );
        return null;
      }
  ...
  public isAcceleratedBlock(
    ...
    for (let i = 0; i < accelerations.length && !anyAccelerated; i++) {
      anyAccelerated =
        anyAccelerated ||
        accelerations[i].pools?.includes(block.extras.pool.id);
    }
    return anyAccelerated;
  }

  // get a list of accelerations that have changed between two sets of accelerations
  public getAccelerationDelta(
    oldAccelerationMap: Record<string, Acceleration>,
    ...
    for (const acceleration of Object.values(newAccelerationMap)) {
      ...
      if (oldAccelerationMap[acceleration.txid] == null) {
        ...
        if (
          oldAccelerationMap[acceleration.txid].feeDelta !==
          acceleration.feeDelta
        ) {
          // feeDelta changed
          changed.push(acceleration.txid);
      ...
      if (!this.ws) {
        ...
        this.ws.on('message', (data, isBinary) => {
          try {
            ...
            logger.warn(
              'Failed to parse acceleration websocket message: ' +
                (e instanceof Error ? e.message : e)
            );
          }
        });
        ...
        if (
          ...
          Date.now() - this.lastPing > 10000
        ) {
          logger.warn(
            'No pong received within 10 seconds, terminating connection'
          );
          try {
            this.ws?.terminate();
          } catch (e) {
            logger.warn(
              'failed to terminate acceleration websocket connection: ' +
                (e instanceof Error ? e.message : e)
            );
          } finally {
            this.ws = null;
          ...
          if (this.ws?.readyState === WebSocket.OPEN) {
            try {
              ...
              logger.warn(
                'failed to send ping to acceleration websocket server: ' +
                  (e instanceof Error ? e.message : e)
              );
            }
          }
