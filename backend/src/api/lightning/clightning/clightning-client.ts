...
import {
  convertAndmergeBidirectionalChannels,
  convertNode,
} from './clightning-convert';
...
export default class CLightningClient
  extends EventEmitter
  implements AbstractLightningApi
{
  ...
  constructor(rpcPath = defaultRpcPath) {
    ...
    if (!fExists(rpcPath) || !fStat(rpcPath).isSocket()) {
      ...
      // to be removed in v0.2.0
        ...
        logger.warn(
          `${rpcPath}/lightning-rpc is missing, using the bitcoin mainnet subdirectory at ${rpcPath}/bitcoin instead.`,
          logger.tags.ln
        );
        logger.warn(
          `specifying the main lightning data directory is deprecated, please specify the network directory explicitly.\n`,
          logger.tags.ln
        );
    ...
    this.client = createConnection(rpcPath).on('error', () => {
      _self.increaseWaitTime();
      _self.reconnect();
    });
    this.rl = createInterface({ input: this.client }).on('error', () => {
      _self.increaseWaitTime();
      _self.reconnect();
    });
    ...
    this.clientConnectionPromise = new Promise<void>((resolve) => {
      ...
      _self.client.on('end', () => {
        logger.err(
          `CLightning client connection closed, reconnecting`,
          logger.tags.ln
        );
        _self.increaseWaitTime();
        _self.reconnect();
      ...
      _self.client.on('error', (error) => {
        logger.err(
          `CLightning client connection error: ${error}`,
          logger.tags.ln
        );
        _self.increaseWaitTime();
        _self.reconnect();
  ...
  async $getNetworkGraph(): Promise<ILightningApi.NetworkGraph> {
    ...
    const channelsList = await convertAndmergeBidirectionalChannels(
