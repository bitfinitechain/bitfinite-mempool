...
class NodesRecordsRepository {
  ...
  public async $getRecordTypes(publicKey: string): Promise<any> {
    try {
      ...
      logger.err(
        `Cannot retrieve custom records for ${publicKey} from db. Reason: ` +
  ...
  public async $deleteUnusedRecords(
    publicKey: string,
    recordTypes: number[]
  ): Promise<number> {
    try {
      ...
      logger.err(
        `Cannot delete unused custom records for ${publicKey} from db. Reason: ` +
          (e instanceof Error ? e.message : e)
      );
      return 0;
    }
