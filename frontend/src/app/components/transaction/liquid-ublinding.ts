...
export class LiquidUnblinding {
  ...
  deduceBlinded(tx: any) {
    ...
    const unknownIns = tx.vin.filter(
      (vin) => vin.prevout && vin.prevout.value == null
    );
    ...
    if (unknownIns.length + unknownOuts.length === 1) {
      ...
      tx.vin
        ...
        .forEach(({ prevout }) =>
          totals.set(
            prevout.asset,
            (totals.get(prevout.asset) || 0) + prevout.value
          )
        );
      tx.vout
      ...
      const remainder = Array.from(totals.entries()).filter(
        ([asset, value]) => value !== 0
      );
