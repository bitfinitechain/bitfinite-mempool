...
@Component({
  ...
  styles: [
    `
      ::ng-deep .tx-wrapper-tooltip-chart {
        width: 135px;
      }
      .loadingGraphs {
        position: absolute;
...
export class LbtcPegsGraphComponent implements OnInit, OnChanges {
  ...
  ngOnChanges() {
    ...
    this.subscription = this.stateService.conversions$
      .pipe(
        switchMap((conversions) =>
          this.priceService
            .getPriceByBulk$(
              this.data.liquidPegs.labels
                .map((date: string) =>
            ...
            .pipe(
              map((prices) =>
                this.data.liquidReserves.series.map(
                  (value, i) =>
                    value * (prices[i]?.price.USD || conversions['USD'])
                )
              )
            )
        )
      ...
      .subscribe((usdBanlance: any) => {
        if (!this.data.liquidReserves) {
          this.pegsChartOptions = this.createChartOptions(
            this.data.liquidPegs.series,
            this.data.liquidPegs.labels
          );
        } else {
          this.pegsChartOptions = this.createChartOptions(
            this.data.liquidPegs.series,
            this.data.liquidPegs.labels,
            this.data.liquidReserves.series,
            usdBanlance
          );
        }
  ...
  createChartOptions(
    ...
    return {
        ...
        formatter: (params: any) => {
          ...
          for (let index = params.length - 1; index >= 0; index--) {
            ...
            if (index < 26) {
              let formattedValue;
              item.seriesName === 'USD'
                ? (formattedValue = this.amountShortenerPipe.transform(
                    item.value,
                    3,
                    undefined,
                    true,
              ...
              itemFormatted += `<div class="item">
                ...
                <div class="value">${formattedValue} <span class="symbol">${
                  item.seriesName
                }</span></div>
              </div>`;
            }
      ...
      xAxis: {
        ...
        boundaryGap: false,
        data: labels.map(
          (value: any) => `${formatDate(value, 'MMM\ny', this.locale)}`
        ),
      },
      yAxis: [
        {
  ...
  onLegendSelectChanged(e) {
    ...
    this.adjustedLeft =
      this.selected['LBTC'] || this.selected['BTC']
        ? +this.left
        : +this.left - 40;
