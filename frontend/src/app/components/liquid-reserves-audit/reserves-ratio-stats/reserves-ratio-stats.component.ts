...
export class ReservesRatioStatsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.unbackedMonths$ = this.fullHistory$.pipe(
      map((fullHistory) => {
        if (
          fullHistory.liquidPegs.series.length !==
          fullHistory.liquidReserves.series.length
        ) {
          return {
        ...
        let ratioSeries = fullHistory.liquidReserves.series.map(
          (value: number, index: number) =>
            value / fullHistory.liquidPegs.series[index]
        );
