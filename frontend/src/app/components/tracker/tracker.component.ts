...
export class TrackerComponent implements OnInit, OnDestroy {
  ...
  acceleratorAvailable: boolean =
    this.stateService.env.ACCELERATOR && this.stateService.network === '';
  ...
  ngOnInit() {
    ...
    this.acceleratorAvailable =
      this.stateService.env.OFFICIAL_MEMPOOL_SPACE &&
      this.stateService.env.ACCELERATOR &&
        ...
        switchMap((blockHeight: number) => {
          if (
            ...
            return this.servicesApiService
              .getAccelerationDataForTxid$(this.txId)
              .pipe(
                switchMap((accelerationData: Acceleration) => {
                  if (this.tx.acceleration && !accelerationData) {
      ...
      this.stateService.mempoolTxPosition$.subscribe((txPosition) => {
        ...
        if (
          ...
          if (this.tx && !this.tx.status.confirmed) {
            ...
            if (
              this.mempoolPosition.accelerated &&
        ...
        (tx: Transaction) => {
          ...
          if (this.tx?.status?.confirmed) {
            ...
            if (tx.cpfpChecked) {
              ...
              const hasRelatives = !!(
                tx.ancestors?.length || tx.bestDescendant
              );
              this.hasEffectiveFeeRate =
                hasRelatives ||
                (tx.effectiveFeePerVsize &&
                  Math.abs(tx.effectiveFeePerVsize - tx.feePerVsize) > 0.01);
            } else {
              this.fetchCpfp$.next(this.tx.txid);
            }
          }
  ...
  setCpfpInfo(cpfpInfo: CpfpInfo): void {
    ...
    this.hasCpfp = !!(
      this.cpfpInfo &&
      (this.cpfpInfo.bestDescendant ||
        this.cpfpInfo.descendants?.length ||
        this.cpfpInfo.ancestors?.length)
    );
    this.hasEffectiveFeeRate =
      hasRelatives ||
      (this.tx.effectiveFeePerVsize &&
        Math.abs(this.tx.effectiveFeePerVsize - this.tx.feePerVsize) > 0.01);
  }
