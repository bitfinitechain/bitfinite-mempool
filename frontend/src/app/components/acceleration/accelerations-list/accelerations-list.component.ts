  ...
  ngOnInit(): void {
    ...
    if (!this.widget) {
      ...
      this.seoService.setTitle(
        $localize`:@@02573b6980a2d611b4361a2595a4447e390058cd:Accelerations`
      );

      this.paramSubscription = combineLatest([this.route.params, timer(0)])
        .pipe(
          tap(([params]) => {
      ...
      this.keyNavigationSubscription = this.stateService.keyNavigation$
        .pipe(
          ...
          tap((event) => {
            ...
            if (
              event.key === nextKey &&
              this.page * 15 < this.accelerationCount
            ) {
              this.page++;
              this.isLoading = true;
              this.cd.markForCheck();
    ...
    this.accelerationList$ = this.pageSubject.pipe(
      switchMap((page) => {
        this.isLoading = true;
        const accelerationObservable$ =
          this.accelerations$ ||
          (this.pending
            ? this.stateService.liveAccelerations$
            : this.servicesApiService.getAccelerationHistoryObserveResponse$({
                page: page,
              }));
        ...
        return accelerationObservable$.pipe(
          switchMap((response) => {
            ...
            if (response.body) {
              ...
              this.accelerationCount = parseInt(
