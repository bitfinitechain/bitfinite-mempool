...
export class AccelerationFeesGraphComponent
  implements OnInit, OnChanges, OnDestroy
{
  ...
  ngOnInit(): void {
    if (this.widget) {
      ...
      this.seoService.setTitle(
        $localize`:@@bcf34abc2d9ed8f45a2f65dd464c46694e9a181e:Acceleration Fees`
      );
    ...
    this.aggregatedHistory$ = combineLatest([
      merge(
        ...
        switchMap((timespan) => {
          ...
          return this.servicesApiService.getAggregatedAccelerationHistory$({
      ...
      tap(([response]) => {
        ...
        this.daysAvailable =
          (new Date().getTime() / 1000 -
            response.headers.get('x-oldest-accel')) /
          (24 * 3600);
          ...
          : {
              ...
              axisLabel: {
                formatter: (val): string =>
                  formatterXAxisTimeCategory(this.locale, this.timespan, val),
                align: 'center',
                fontSize: 11,
                lineHeight: 12,
                hideOverlap: true,
  ...
  onSaveChart() {
    ...
    download(
      ...
      `acceleration-fees-${this.timespan}-${Math.round(
        now.getTime() / 1000
      )}.svg`
    );
    // @ts-ignore
