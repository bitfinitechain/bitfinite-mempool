...
export class AccelerateCheckout implements OnInit, OnDestroy {
  ...
  moveToStep(step: CheckoutStep, force: boolean = false): void {
    ...
    if (
      !this.estimate &&
      ['quote', 'summary', 'checkout', 'processing'].includes(this.step)
    ) {
      this.fetchEstimate();
    }
  ...
  fetchEstimate(): void {
    ...
    this.quoteError = null;
    this.accelerateError = null;
    this.estimateSubscription = this.servicesApiService
      .estimate$(this.tx.txid)
      .pipe(
        tap((response) => {
          if (response.status === 204) {
            ...
            if (
              this.estimate.hasAccess === true &&
              this.estimate.userBalance <= 0
            ) {
              if (this.isLoggedIn()) {
            ...
            this.etaInfo$ = this.etaService.getProjectedEtaObservable(
              this.estimate,
              this.miningStats
            );

            this.maxRateOptions = this.estimate.options.map(
              (option, index) => ({
                fee: option.fee,
                rate:
                  (this.estimate.txSummary.effectiveFee + option.fee) /
                  this.estimate.txSummary.effectiveVsize,
                index,
              })
            );
            ...
            this.cost =
              this.userBid +
              this.estimate.mempoolBaseFee +
              this.estimate.vsizeFee;
            ...
            if (
              this.step === 'checkout' &&
              this.canPayWithBitcoin &&
              !this.loadingBtcpayInvoice
            ) {
              this.requestBTCPayInvoice();
            }
  ...
  setUserBid({ fee, index }: { fee: number; index: number }): void {
    if (this.estimate) {
      ...
      this.cost =
        this.userBid + this.estimate.mempoolBaseFee + this.estimate.vsizeFee;
      this.validateChoice();
    }
  }
  ...
  accelerateWithMempoolAccount(): void {
    ...
    this.accelerationSubscription = this.servicesApiService
      .accelerate$(this.tx.txid, this.userBid)
      .subscribe({
        next: () => {
          this.processing = false;
      ...
      applePayButton.addEventListener('click', async (event) => {
        ...
        try {
          ...
          if (tokenResult?.status === 'OK') {
            ...
            const cardTag = md5(
              `${card.brand}${card.expMonth}${card.expYear}${card.last4}`.toLowerCase()
            );
            // keep checkout in loading state until the acceleration request completes
            this.isTokenizing++;
            this.isCheckoutLocked++;
            this.servicesApiService
              .accelerateWithApplePay$(
                this.tx.txid,
                tokenResult.token,
                cardTag,
                `accelerator-${this.tx.txid.substring(0, 15)}-${Math.round(
              ...
              .subscribe({
                next: (response) => {
                  ...
                  this.processing = false;
                  this.apiService
                    .logAccelerationRequest$(this.tx.txid)
                    .subscribe();
                  this.audioService.playSound('ascend-chime-cartoon');
                  if (this.applePay) {
                    this.applePay.destroy();
                  }
                  this.paymentReceipt.emit(
                    this.accelerationResponse?.receiptUrl
                  );
                  setTimeout(() => {
                    this.isTokenizing--;
                    this.isCheckoutLocked--;
  ...
  async requestGooglePayPayment(): Promise<void> {
    ...
    this.googlePay = await this.payments.googlePay(paymentRequest, {
      referenceId: `accelerator-${this.tx.txid.substring(0, 15)}-${Math.round(
        new Date().getTime() / 1000
      )}`,
    });
          ...
          if (tokenResult?.status === 'OK') {
            ...
            if (!verificationToken || !verificationToken.token) {
              ...
              this.accelerateError =
                'SCA Verification Failed. Payment Declined.';
              this.processing = false;
              return;
            }
            const cardTag = md5(
      ...
      this.servicesApiService
        ...
        .subscribe({
          ...
          error: (response) => {
            ...
            this.isTokenizing--;
            if (
              !(response.status === 403 && response.error === 'not_available')
            ) {
              setTimeout(() => {
                // Reset everything by reloading the page :D, can be improved
                const urlParams = new URLSearchParams(window.location.search);
                window.location.assign(
                  window.location
                    .toString()
                    .replace(
                      `?cash_request_id=${urlParams.get('cash_request_id')}`,
                      ``
                    )
                );
              }, 3000);
  ...
  async $verifyBuyer(
    ...
    details,
    amount
  ): Promise<{ token: string; userChallenged: boolean }> {
    const verificationDetails = {
      amount: amount,
      currencyCode: 'USD',
      intent: 'CHARGE',
  ...
  async requestBTCPayInvoice(): Promise<void> {
    ...
    this.servicesApiService
      .generateBTCPayAcceleratorInvoice$(this.tx.txid, this.userBid)
      .pipe(
        switchMap((response) => {
          return this.servicesApiService.retrieveInvoice$(
            response.btcpayInvoiceId
          );
        }),
        catchError((error) => {
          console.log(error);
  ...
  get paymentMethods(): PaymentMethod[] {
    return Object.keys(
      this.estimate?.availablePaymentMethods || {}
    ) as PaymentMethod[];
  }
  ...
  get couldPay(): boolean {
    return (
      this.couldPayWithBalance ||
      this.couldPayWithBitcoin ||
      this.couldPayWithCashapp ||
      this.couldPayWithApplePay ||
      this.couldPayWithGooglePay
    );
  }
  ...
  get canPayWithBitcoin(): boolean {
    ...
    return (
      paymentMethod &&
      this.cost >= paymentMethod.min &&
      this.cost <= paymentMethod.max
    );
  }
  ...
  get canPayWithBalance(): boolean {
    ...
    return (
      ...
      this.cost >= paymentMethod.min &&
      this.cost <= paymentMethod.max &&
      this.cost <= this.estimate?.userBalance
    );
  }

  get canPay(): boolean {
    return (
      this.canPayWithBalance ||
      this.canPayWithBitcoin ||
      this.canPayWithCashapp ||
      this.canPayWithApplePay ||
      this.canPayWithGooglePay
    );
  }
