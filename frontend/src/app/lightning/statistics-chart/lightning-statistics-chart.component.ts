...
export class LightningStatisticsChartComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    if (this.widget) {
      ...
      this.seoService.setTitle(
        $localize`:@@ea8db27e6db64f8b940711948c001a1100e5fe9f:Lightning Network Capacity`
      );
      this.seoService.setDescription(
        $localize`:@@meta.description.lightning.stats-chart:See the capacity of the Lightning network visualized over time in terms of the number of open channels and total bitcoin capacity.`
      );
    ...
    this.capacityObservable$ = this.radioGroupForm
      .get('dateSpan')
      .valueChanges.pipe(
        ...
        switchMap((timespan) => {
          ...
          return this.lightningApiService
            .cachedRequest(
              this.lightningApiService.listStatistics$,
              250,
              timespan
            )
            .pipe(
              tap((response: any) => {
                ...
                this.chartData = {
                  channel_count: data.map((val) => [
                    val.added * 1000,
                    val.channel_count,
                  ]),
                  capacity: data.map((val) => [
                    val.added * 1000,
                    val.total_capacity,
                  ]),
                };
  ...
  prepareChartOptions(data): void {
    ...
    this.chartOptions = {
      ...
      legend:
        this.widget || data.channel_count.length === 0
          ? undefined
          : {
              padding: 10,
              data: [
  ...
  onSaveChart(): void {
    ...
    download(
      ...
      `lightning-network-capacity-${this.timespan}-${Math.round(
        now.getTime() / 1000
      )}.svg`
    );
    // @ts-ignore
