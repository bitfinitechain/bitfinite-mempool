    ...
    this.channel$ = this.activatedRoute.paramMap.pipe(
      switchMap((params: ParamMap) => {
        ...
        this.ogSession = this.openGraphService.waitFor(
          'channel-map-' + this.shortId
        );
        this.ogSession = this.openGraphService.waitFor(
          'channel-data-' + this.shortId
        );
        this.error = null;
        this.seoService.setTitle(`Channel: ${params.get('short_id')}`);
        this.seoService.setDescription(
          $localize`:@@meta.description.lightning.channel:Overview for Lightning channel ${params.get(
            'short_id'
          )}. See channel capacity, the Lightning nodes involved, related on-chain transactions, and more.`
        );
