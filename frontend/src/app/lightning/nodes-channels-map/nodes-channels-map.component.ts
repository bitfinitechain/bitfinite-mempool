...
export class NodesChannelsMap implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.style === 'graph') {
      ...
      this.seoService.setTitle(
        $localize`:@@b482ceceb39c7a045cb2ab2c64f7091d21e63d44:Lightning Nodes Channels World Map`
      );
      this.seoService.setDescription(
        $localize`:@@meta.description.lightning.node-map:See the channels of non-Tor Lightning network nodes visualized on a world map. Hover/tap on points on the map for node names and details.`
      );
    }
    ...
    this.channelsObservable = this.activatedRoute.paramMap.pipe(
      delay(100),
      switchMap((params: ParamMap) => {
        this.isLoading = true;
        if (
          (this.style === 'channelpage' && this.channel.length === 0) ||
          !this.hasLocation
        ) {
          ...
          tap((data) => {
            ...
            for (const channel of geoloc) {
              ...
              if (this.style === 'widget') {
                ...
                node2GpsLgt = 3;
                node1UniqueId =
                  channel[node1GpsLat].toString() +
                  channel[node1GpsLgt].toString();
                node2UniqueId =
                  channel[node2GpsLat].toString() +
                  channel[node2GpsLgt].toString();
              }
          ...
          itemStyle: {
            color: (params) => {
              ...
              return `${lerpColor(
                '#1E88E5',
                '#D81B60',
                Math.pow(params.data[5] / maxLiquidity, 0.2)
              )}`;
            },
            opacity: 1,
            borderColor: 'black',
  ...
  onChartInit(ec) {
      ...
      this.chartInstance.getZr().on('click', (e) => {
        this.zone.run(() => {
          const url = new RelativeUrlPipe(this.stateService).transform(
            `/graphs/lightning/nodes-channels-map`
          );
    ...
    this.chartInstance.on('georoam', (e) => {
      ...
      chartOptions.series[1].lineStyle.opacity = Math.max(
        0.05,
        Math.min(0.5, chartOptions.series[1].lineStyle.opacity)
      );
      chartOptions.series[1].lineStyle.width = Math.max(
        0.5,
        Math.min(1, chartOptions.series[1].lineStyle.width)
      );
