...
export class DocsComponent implements OnInit {
  ...
  ngDoCheck(): void {
    ...
    if (url[0].path === 'faq') {
      ...
      this.seoService.setDescription(
        $localize`:@@meta.description.docs.rest-bitcoin:Documentation for the mempool.bitfinitechain.org REST API service: get info on addresses, transactions, blocks, fees, mining, and more.`
      );
      ...
      this.seoService.setDescription(
        $localize`:@@meta.description.docs.websocket-bitcoin:Documentation for the mempool.bitfinitechain.org WebSocket API service: get real-time info on blocks, mempools, transactions, addresses, and more.`
      );
    } else {
      this.activeTab = 3;
      this.seoService.setTitle(
