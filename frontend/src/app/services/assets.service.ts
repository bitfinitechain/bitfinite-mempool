...
export class AssetsService {
  ...
  constructor(
    ...
    this.getAssetsJson$ = this.stateService.networkChanged$.pipe(
      switchMap(() =>
        this.httpClient.get(
          `${apiBaseUrl}/resources/assets${
            this.stateService.network === 'liquidtestnet' ? '-testnet' : ''
          }.json`
        )
      ),
      map((rawAssets) => {
    ...
    this.getAssetsMinimalJson$ = this.stateService.networkChanged$.pipe(
      switchMap(() =>
        this.httpClient.get(
          `${apiBaseUrl}/resources/assets${
            this.stateService.network === 'liquidtestnet' ? '-testnet' : ''
      ...
      map((assetsMinimal) => {
        if (this.stateService.network === 'liquidtestnet') {
          ...
          ] = [null, 'tLBTC', 'Test Liquid Bitcoin', 8];
        }
    ...
    this.getWorldMapJson$ = this.httpClient
      .get(apiBaseUrl + '/resources/worldmap.json')
      .pipe(shareReplay());
  }
}
